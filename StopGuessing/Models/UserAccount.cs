...
namespace StopGuessing.Models
{
    ...
    public interface IUserAccount : IDisposable
    {
        ...
        /// </summary>
        byte[] SaltUniqueToThisAccount { get; set; }
        ...
        bool HasClientWithThisHashedCookieSuccessfullyLoggedInBefore(string hashOfCookie);
        void RecordHashOfDeviceCookieUsedDuringSuccessfulLogin(string hashOfCookie, DateTime? whenSeenUtc = null);

        bool AddIncorrectPhase2Hash(string phase2Hash, DateTime? whenSeenUtc = null);
    ...
    public static class UserAccountController
    {
            ...
            int? numberOfIterationsToUseForPhase1Hash = null)
        {
            ...
            if (oldPassword != null &&
                ComputePhase2HashFromPhase1Hash(oldPasswordHashPhase1 = ComputePhase1Hash(userAccount, oldPassword)) ==
                userAccount.PasswordHashPhase2)
            {
        ...
        {
            double amountAvailable = userAccount.CreditLimit - userAccount.GetCreditsConsumed(timeOfRequestUtc);
            ...
            int? saltLength = null)
        {
            ...
            if (creditLimit.HasValue)
                userAccount.CreditLimit = creditLimit.Value;
            else if (userAccount.CreditLimit < 0)
                userAccount.CreditLimit = DefaultCreditLimit;

            if (creditHalfLife.HasValue)
                userAccount.CreditHalfLife = creditHalfLife.Value;
            else if (userAccount.CreditHalfLife == TimeSpan.Zero)
                userAccount.CreditHalfLife = new TimeSpan(DefaultCreditHalfLifeInHours, 0, 0);

            if (saltUniqueToThisAccount != null)
            {
                userAccount.SaltUniqueToThisAccount = saltUniqueToThisAccount;
            }
            else if (userAccount.SaltUniqueToThisAccount == null)
            {
                userAccount.SaltUniqueToThisAccount = new byte[saltLength ?? DefaultSaltLength];
                StrongRandomNumberGenerator.GetBytes(userAccount.SaltUniqueToThisAccount);
            }

            if (phase1HashFunctionName != null)
                userAccount.PasswordHashPhase1FunctionName = phase1HashFunctionName;
            else if (userAccount.PasswordHashPhase1FunctionName == null)
                userAccount.PasswordHashPhase1FunctionName = ExpensiveHashFunctionFactory.DefaultFunctionName;

            if (numberOfIterationsToUseForPhase1Hash > 0)
                userAccount.NumberOfIterationsToUseForPhase1Hash = numberOfIterationsToUseForPhase1Hash ?? DefaultIterationsForPasswordHash;
            else if (userAccount.NumberOfIterationsToUseForPhase1Hash == 0)
                userAccount.NumberOfIterationsToUseForPhase1Hash = ExpensiveHashFunctionFactory.DefaultNumberOfIterations;
